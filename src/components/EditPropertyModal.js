...
const EditPropertyModal = ({ property, onClose, onUpdate }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [landlordsResponse, bankResponse] = await Promise.all([
          ...
          axios.get(`${process.env.REACT_APP_API_URL}/api/bankaccounts`, { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.address || !formData.rent_amount || !formData.landlord_id) {
  ...
  const handleCancel = () => {
    console.log('Edit property modal cancel clicked');
    if (typeof onClose === 'function') {
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start sm:items-center justify-center z-50 overflow-y-auto py-4 sm:py-0">
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg w-full max-w-xs sm:max-w-md mx-4 sm:mx-0">
        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Edit Property</h3>
